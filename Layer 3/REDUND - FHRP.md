...
| Protocol | syntax         | IP address                       | MAC address                              | Cisco Proprietary? |
| -------- | -------------- | -------------------------------- | ---------------------------------------- | ------------------ |
| HSRP     | active/standby | V1: 224.0.0.2<br>V2: 224.0.0.102 | V1: 0000.0c07.acxx<br>V2: 0000.0c9F.Fxxx | ye                 |
| VRRP     | mastery/backup | 224.0.0.18                       | 000.5e00.01xx                            | no                 |
| GLBP     | AVG/AVE        | 224.0.0.102                      | 0007.b400.xxyy                           | yes                |

## Troubleshooting
- preemption is not enabled by default
- Make sure to increase priority if you want preemption
...
- computer networking protocol designed to protect the default gateway used on a subnetwork by allowing two or more routers to provide backup for that address
- in the event of a failure of an active router, the backup router will take over the address, usually within a few seconds
- virtual IP is configured on the routers, and a virtual MAC is generated for the virtual IP (each FHRP uses a different format for the virtual MAC)
...
- note that FHRP activated routers communicate with each other by sending hello multicast messages

| number | step                                                                                                                                                                                               | additional info                                                                                                                                                                                                                                  |
| ------ | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| 1      | virtual IP is configured on the routers and generate virtual MAC for the virtual IP                                                                                                                | Configure a default virtual IP address for the default gateway                                                                                                                                                                                   |
| 2      | active and standby elated                                                                                                                                                                          | Router will negotiate which one becomes the default gateway                                                                                                                                                                                      |
| 3      | end hosts config to use virtual IP address                                                                                                                                                         |                                                                                                                                                                                                                                                  |
| 4      | active router replies to ARP requests using the virtual MAC aaddress                                                                                                                               |                                                                                                                                                                                                                                                  |
| 5      | If active R fails, standby becomes next active router <br><br>new active router sends gratitous ARP to update the MAC table of the other switches<br><br>Non-preemptive in case old router returns | Gratuitous ARP sent withoout being requested (broadcasted) to update all switches mac-address tables<br><br>HRP is non -preemptive. The current active routers will not automatically give up its role, even if the former active router returns |
...
### The HSRP active router determination order (PI)
1. Highest priority
...
| 4      | active router determined by highest priority (default 100)                                                                                                                | R(config-if)# standby <0-255> priority <0-255> |
